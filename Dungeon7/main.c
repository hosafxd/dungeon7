...
#include "ITEM.h"
#include "MONSTER.h"
#include <time.h> // Add at the top of main.c
#include <windows.h>
#include <io.h>
...
typedef struct Player
{
    ...
    int strength;
    int baseAttack;
    Item *inventory; // Capacity of three
} Player;
...
{
    int barWidth = 20; // Adjust for desired size
...
{
    int barWidth = 20; // Adjust as desired
    //int healthPercentage = (monster.health * 100) / monster.maxHealth;
...
// In main(), after loading rooms and monsters:
    int numMonsters;
    Monster *monsters = loadMonsters(&numMonsters);
    ...
    // Free the temporary monsters array??????what happens if not
    free(monsters);
...
void gameLoop(Player *player, Room **rooms, int numRooms)
{
    ...
    Room *lastRoom = NULL; // Track the last displayed room??????
    ...
    while (1)
    {
        ...
        if (!monstersRemain(rooms, numRooms))
        {
            // No monsters left
            clock_t end = clock();
            ...
            // End the game or break out of the main game loop
        ...
        // Check if we are in a new room, player moved
        ...
        //user input ve built in commandleri fonksiyonluyoruz
        if (strncmp(command, "move", 4) == 0)
        {
            moveCommand(player, &currentRoom);
            // Do not print room info here. On next loop iteration,
            // if currentRoom changed, it will print automatically.
        }
...
void moveCommand(Player *player, Room **currentRoom)
{
    ...
    // Adjust these ASCII maps as you like, they are just examples:
...
void lookCommand(Player *player, Room **currentRoom)
{
    ...
    // Doors (Directions)
    // Even though movement is done by directions now, we can still show the available doors here.
...
void inventoryCommand(Player *player, Room *currentRoom)
{
    ...
    if (choice > 0 && choice <= count)
    {
        ...
        // Check if it's a healthpot or another item
        if (strcmp(selectedItem->name, "potion") == 0)
        {
            ...
            {
                // Use the potion
                int healAmount = 15; // assuming +15 health from potion
        ...
        {
            // For dropping non-healthpot items
            ...
            if (strcmp(input, "yes") == 0)
            {
                ...
                *current = (*current)->next;
                ...
                // Do not free the item now since it goes back into the room
            }
            else
            {
...
void pickupCommand(Player *player, Room *currentRoom, char *itemName)
{
    ...
    *currentItem = (*currentItem)->next; // Remove from room
    ...
    {
        // For other items that have health boosts, attack boosts, strength boosts:
        // Update maxHealth, currentHealth, attack, and strength as per normal items

        // Increase maxHealth by the health boost
        player->maxHealth += itemToPickup->healthBoost;
        // Also increase currentHealth by the same amount
        player->health += itemToPickup->healthBoost;
        // Increase attack and strength
        player->baseAttack += itemToPickup->attackBoost;
        player->strength += itemToPickup->strengthBoost;

        // Ensure currentHealth does not exceed maxHealth (though it shouldn't now)
        if (player->health > player->maxHealth)
        {
            player->health = player->maxHealth;
        }
...
void restoreConnections(Room **rooms, int numRooms) {
    // Same logic as before, just ensuring it is called after loading
    for (int i = 0; i < numRooms; i++) {
        if (strcmp(rooms[i]->name, "HUMAN SHELF") == 0) {
